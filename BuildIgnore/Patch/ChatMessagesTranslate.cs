...
namespace FargoChinese.BuildIgnore.Patch
{
    public class ChatMessagesTranslate : PatchBase
    {
        ...
        {
            On_Main.NewText_string_byte_byte_byte += NewText_string_byte_byte_byte;
            On_Main.NewTextMultiline += NewTextMultiline;
        }
        private static void NewText_string_byte_byte_byte(On_Main.orig_NewText_string_byte_byte_byte orig, string newText, byte R = byte.MaxValue, byte G = byte.MaxValue, byte B = byte.MaxValue)
        {
            if (newText == null)
                return;
            #region Fargowiltas
        ...
        private static void NewTextMultiline(On_Main.orig_NewTextMultiline orig, string text, bool force = false, Color c = default, int WidthLimit = -1)
        {
            if (text == null)
                return;
            #region Fargowiltas
